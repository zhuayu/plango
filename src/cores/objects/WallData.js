...
const WallData = function() {
  this.m_vStart; // 启点
  this.m_vEnd; // 终点
  this.m_vCenter; // 中心点
  this.m_fLength; // 长度
  this.m_iType = 0; // 0 墙中线，1内墙线，2外墙线
  this.m_fWidth = 20;
  ...
  this.OnInit = function(x1, y1, iType) {
    ...
    if (this.m_iType == 0)
      // 0 墙中线，1内墙线，2外墙线
      this.m_fValue = 0;
    if (this.m_iType == 1) this.m_fValue = 0.5;
    if (this.m_iType == 2) this.m_fValue = -0.5;
    ...
    geom.faceVertexUvs[0][0] = [
      new THREE.Vector2(0, 0),
      new THREE.Vector2(0, 0),
      new THREE.Vector2(0, 0)
    ];
    geom.faceVertexUvs[0][1] = [
      new THREE.Vector2(0, 0),
      new THREE.Vector2(0, 0),
      new THREE.Vector2(0, 0)
    ];
    ...
    geometry_2.vertices.push(
      new THREE.Vector3(-0.5, -0.5 + this.m_fValue, 0.1),
      new THREE.Vector3(0.5, -0.5 + this.m_fValue, 0.1)
    );
    geometry_2.vertices.push(
      new THREE.Vector3(0.5, -0.5 + this.m_fValue, 0.1),
    ...
    var tEdge = new THREE.LineSegments(
      geometry_2,
      new THREE.LineBasicMaterial({ color: 0x999999, opacity: 1 })
    );

    this.mWall.add(tEdge);
    var sphere = new THREE.CircleGeometry(9, 9);
    var tMat = new THREE.PointsMaterial({ color: 0xeeeeee });
    ...
    geometry.vertices.push(
      ...
      new THREE.Vector3(100 / 2, 0, 0),
      new THREE.Vector3(-100 / 2, -20, 0),
      new THREE.Vector3(-100 / 2, 20, 0),
      new THREE.Vector3(100 / 2, -20, 0),
      new THREE.Vector3(100 / 2, 20, 0)
    );
    this.mLabel = new THREE.LineSegments(
      geometry,
  ...
  this.DeDaoZhongXian = function() {
    ...
    if (f1 > f2) {
      var vec1 = new THREE.Vector3(
        (this.m_LastLineArray[0].x + this.m_LastLineArray[3].x) / 2,
        (this.m_LastLineArray[0].y + this.m_LastLineArray[3].y) / 2,
        0
      );
      var vec2 = new THREE.Vector3(
        (this.m_LastLineArray[1].x + this.m_LastLineArray[2].x) / 2,
        (this.m_LastLineArray[1].y + this.m_LastLineArray[2].y) / 2,
        0
      );
      tArray.push(vec1);
      tArray.push(vec2);
      ...
      var vec3 = new THREE.Vector3(
        (this.m_LastLineArray[0].x + this.m_LastLineArray[1].x) / 2,
        (this.m_LastLineArray[0].y + this.m_LastLineArray[1].y) / 2,
        0
      );
      var vec4 = new THREE.Vector3(
        (this.m_LastLineArray[2].x + this.m_LastLineArray[3].x) / 2,
        (this.m_LastLineArray[2].y + this.m_LastLineArray[3].y) / 2,
        0
      );
      tArray.push(vec3);
      tArray.push(vec4);
  ...
  this.OnUpdate = function() {
    //更新墙体
    this.m_fLength = Math.sqrt(
      (this.m_vEnd.x - this.m_vStart.x) * (this.m_vEnd.x - this.m_vStart.x) +
    ...
    if (Math.abs(edge1.x) < 0.001) edge1.x = 0.0;
    if (Math.abs(edge1.y) < 0.001) edge1.y = 0.0;

    if (edge1.x == 0.0 && edge1.y == 0.0)
      // atanf(0/0)�����ֵ
      this.m_fRotate = 0.0;
    else this.m_fRotate = Math.atan(edge1.y / edge1.x);
  ...
  this.OnRender = function() {
    ...
    this.m_WallLineArray[0] = this.m_WallLineArray[0].applyMatrix4(
      this.mWall.matrix
    );
    this.m_WallLineArray[1] = this.m_WallLineArray[1].applyMatrix4(
      this.mWall.matrix
    );
    this.m_WallLineArray[2] = this.m_WallLineArray[2].applyMatrix4(
      this.mWall.matrix
    );
    this.m_WallLineArray[3] = this.m_WallLineArray[3].applyMatrix4(
      this.mWall.matrix
    );
  ...
  this.UpdateLabel = function(iMode) {
    ...
    this.mLabel.geometry.vertices.push(
      new THREE.Vector3(-this.m_fLength / 2, 0, 0),
      new THREE.Vector3(this.m_fLength / 2, 0, 0),
      new THREE.Vector3(-this.m_fLength / 2, -10, 0),
      new THREE.Vector3(-this.m_fLength / 2, 10, 0),
      new THREE.Vector3(-this.m_fLength / 2 - 7, -8, 0),
      new THREE.Vector3(-this.m_fLength / 2 + 7, 8, 0),
      new THREE.Vector3(this.m_fLength / 2 - 7, -8, 0),
      new THREE.Vector3(this.m_fLength / 2 + 7, 8, 0),
      new THREE.Vector3(this.m_fLength / 2, -10, 0),
    ...
    if (iMode == 0) {
      ...
      var tmpMatrix3 = new THREE.Matrix4().makeTranslation(
        this.m_vCenter.x,
        this.m_vCenter.y,
        this.m_vCenter.z
      );
